...
func Display(name string, x interface{}) {
	...
	displayLimited(name, reflect.ValueOf(x), 0)
}
...
func displayLimited(path string, v reflect.Value, loop int) {
	loop++
	...
	case reflect.Slice, reflect.Array:
		for i := 0; i < v.Len(); i++ {
			displayLimited(fmt.Sprintf("%s[%d]", path, i), v.Index(i), loop)
		}
	case reflect.Struct:
		for i := 0; i < v.NumField(); i++ {
			fieldPath := fmt.Sprintf("%s.%s", path, v.Type().Field(i).Name)
			displayLimited(fieldPath, v.Field(i), loop)
		}
	case reflect.Map:
		for _, key := range v.MapKeys() {
			...
			displayLimited(fmt.Sprintf("%s[%s]", path,
	...
	case reflect.Ptr:
		if v.IsNil() {
			...
			displayLimited(fmt.Sprintf("(*%s)", path), v.Elem(), loop)
		}
	case reflect.Interface:
		if v.IsNil() {
			...
			fmt.Printf("%s.type = %s\n", path, v.Elem().Type())
			displayLimited(path+".value", v.Elem(), loop)
		}
